...
def storeDatas(title: str):
    cur = conn.cursor()
    cur.execute("SELECT IsEnter FROM CarStatus WHERE Title = (?) ORDER BY ID DESC LIMIT 1", (title,))
    ...
    if enter_status == None:
        cur.execute(
            ...
            VALUES(?,?,?)", (title, True, _dateset)
        )
    else:
        cur.execute(
            "INSERT INTO CarStatus(Title, IsEnter, DetectTime) \
            VALUES(?,?,?)", (title, not enter_status[0], _dateset)
        )
...
def analyseImg():
    ...
    if object[0] != "Nothing": storeDatas(object[0])
